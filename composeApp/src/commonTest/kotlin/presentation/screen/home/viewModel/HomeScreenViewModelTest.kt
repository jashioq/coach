...
class HomeScreenViewModelTest {
    ...
    @Test
    fun `updates userName in state on init`() = runTest {
        // GIVEN initial state and emitUserNamePreferenceUseCase responds with success
        val testName = "Test User"
        everySuspend { emitUserNamePreferenceUseCase.call(Unit) } returns Result.success(flowOf(testName))
    ...
    @Test
    fun `updates goalId goalName and goalFrequency in state on init`() = runTest {
        // GIVEN initial state and emitAllGoalsUseCase responds with success
        val testGoal = Goal(id = 1, name = "Test Goal", frequency = 7)
        everySuspend { emitUserNamePreferenceUseCase.call(Unit) } returns Result.success(emptyFlow())
        everySuspend { emitAllGoalsUseCase.call(Unit) } returns Result.success(flowOf(listOf(testGoal)))
        viewModel = HomeScreenViewModel(emitUserNamePreferenceUseCase, emitAllGoalsUseCase)
